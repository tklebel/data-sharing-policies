...
sc <- spark_connect(master = "yarn", config = config,
                    app_name = paste0("funding_selectivity_", incentive_setting)
)
...
summarised_data <- df %>% 
  filter(step > 0, sharingincentive == incentive_setting) %>% 
...
plot_skeleton <- function(df, mean_var, sd_var, interval_type = c("sd", "ci")) {
  ...
  df %>%  
    ...
    geom_line(aes(y = {{ mean_var }}, colour = as.factor(fundedshare))) +
