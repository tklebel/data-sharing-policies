...
We look first at the initial stages of the simulation.
...
#| label: fig-corrs-low-clustering-network
#| fig-cap: "Path dependency with low clustering network condition. (A) Pearson correlation
#| between current funding status (yes/no) and funding status at t-1. (B) Pearson 
#| correlation between current funding status and initial resources."
...
```{r, fig.width=8, fig.height=4}
#| label: fig-corrs-no-network
#| fig-cap: "Path dependency under 'no network' condition. (A) Pearson correlation
#| between current funding status (yes/no) and funding status at t-1. (B) Pearson 
#| correlation between current funding status and initial resources."
...
funding_vs_sharing <- funding_vs_sharing %>% 
  mutate(network = "Low clustering network")
```

```{r, fig.width=5, fig.height=4}
#| label: corr-funding-sharing-low-clustering
#| fig-cap: "Correlation between funding status (yes/no) and data sharing (yes/no)"
...
```{r, fig.width=5, fig.height=4}
#| label: corr-low-cluster-sharing-sharing-lag
#| fig-cap: "Correlation between current sharing status (yes/no) and lagged (t-1)
#| sharing status for the case of low clustering."
...
funding_vs_sharing_no_network <- funding_vs_sharing_no_network %>% 
  mutate(network = "No network")
```

```{r, fig.width=5, fig.height=4}
#| label: corr-funding-sharing-no-network
#| fig-cap: "Correlation between funding status (yes/no) and data sharing (yes/no)"
...
```{r, fig.width=5, fig.height=4}
#| label: corr-no-network-sharing-sharing-lag
#| fig-cap: "Correlation between current sharing status (yes/no) and lagged (t-1)
#| sharing status for the case of no network."
