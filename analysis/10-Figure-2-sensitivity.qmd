---
title: "Sensitivity analysis - Figure 2"
date: today
...
# Sharing costs
## Panel A
In Panel A, we fix the share of funded teams at 10%. In the three 
figures below, we show all combinations. The direct comparison to the figure
in the main manuscript is the fith column (.4).
...
```{r}
panel_a <- function(df, outcome, sensitivity_var, title,
                    use_factor = TRUE) {
  
  if (use_factor) {
    df <- mutate(df, {{sensitivity_var}} := as.factor({{sensitivity_var}}))
  } 
  
  df %>% 
    ggplot(aes(step, {{outcome}}, colour = {{sensitivity_var}}, 
               group = {{sensitivity_var}})) +
    geom_rect(data = data.frame(
        fundedshare = unique(summarised_table$fundedshare),
        sharingincentive = .4
      ),
...
### Rate of sharing
```{r}
panel_a(summarised_table, mean_sharing, sharingcostscap,
        title = "Outcome: mean rate of sharing")
```

### Current gini
```{r}
panel_a(summarised_table, mean_gini, sharingcostscap,
        title = "Outcome: mean current gini")
```

### Total Gini
```{r}
panel_a(summarised_table, mean_cumulative_gini, sharingcostscap,
        title = "Outcome: mean total gini")
```


## Panel B
Here we fix the sharing incentive to 40%, as in the original figure. Note
that the x-axis here shows steps 1-3000, in the main manuscript for space reasons
only 1-1500.
...
panel_b <- function(outcome = c("costs", "sigma", "gain")) {
  ...
  no_network_intervention <- sharing_costs_individuals %>% 
    filter(sharingincentive == .4, network == "none")
  
  no_network_effort <- no_network_intervention %>% 
    group_by(step, fundedshare, {{variable_symbol}}, funded) %>% 
  ...
  no_network_effort %>% 
    mutate(
      ...
      fundedshare = paste0(
        "Incentive: ", scales::percent(fundedshare)
      )) %>% 
...
panel_c <- function(outcome = c("costs", "sigma", "gain")) {
  ...
  correlations_no_network <- no_network_intervention %>% 
    group_by(step, fundedshare, {{variable_symbol}}, run_number) %>% 
    ...
    group_by(step, fundedshare, {{variable_symbol}}) %>% 
...
The manuscript uses .15 for sigma.
...
### Rate of sharing
```{r}
panel_a(summarised_table, mean_sharing, proposalsigma,
        title = "Outcome: mean rate of sharing")
```

### Current gini
```{r}
panel_a(summarised_table, mean_gini, proposalsigma,
        title = "Outcome: mean current gini")
```

### Total Gini
```{r}
panel_a(summarised_table, mean_cumulative_gini, proposalsigma,
        title = "Outcome: mean total gini")
```
...
The gain value used in the manuscript is 1.
