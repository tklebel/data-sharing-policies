...
fig-dpi: 300
params:
  sharing_incentive: 0.4
---

This notebook was rendered with sharing_incentive = `r params$sharing_incentive`.
...
# need to evaluate the parameter, to avoid hick-ups with dbplyr
# (evaluating params$sharing_incentive within `filter` did not work)
incentive_setting <- params$sharing_incentive
...
sc <- spark_connect(master = "yarn", config = config,
                    app_name = paste0("funding_selectivity_individual_data_",
                                      incentive_setting)
)
...
fragmented_intervention <- fragmented %>% 
  filter(sharingincentive == incentive_setting, maxinitialutility == -4)
...
low_degree_hypothesis <- fragmented_intervention %>% 
  ...
  summarise(mean_frac_low_degree_funded = mean(fraction),
            sd_frac_low_degree_funded = sd(fraction)) %>% 
  collect()
```


```{r}
skeleton_two <- function(df, var, sd_var) {
  df %>% 
    mutate(fundedshare = scales::percent(fundedshare, accuracy = 1)) %>% 
    ggplot(aes(step, group = fundedshare)) +
    geom_ribbon(aes(ymin = {{ var }} - {{ sd_var }},
                    ymax = {{ var }} + {{ sd_var }}), 
                  fill = "grey40", alpha = .15, show.legend = FALSE) +
    geom_line(aes(y = {{ var }}, colour = fundedshare)) +
...
p1 <- skeleton_two(low_degree_hypothesis, mean_frac_low_degree_funded, 
                   sd_frac_low_degree_funded) +
  scale_y_continuous(breaks = c(true_fraction, seq(.4, to = 1, by = .1)),
                     labels = scales::label_percent()) +
  annotate("text", x = 2200, y = .34, label = "Expected fraction") +
...
low_degree_hypothesis_sharing <- fragmented_intervention %>% 
  ...
  summarise(mean_frac_low_degree_sharing = mean(fraction),
            sd_frac_low_degree_sharing = sd(fraction)) %>% 
  collect()
```
